...
cardiff_oa_boundaries <- bi_class(cardiff_oa_boundaries, x=ProportionPrivateRented, y=ProportionSocialRented) %>%
  ...
  mutate(
    label = paste0(
      ...
      label_percent(accuracy=1, trim=FALSE)(ProportionSocialRented), " social landlords", "<br/>",
      "<hr/>",
      label_percent(accuracy=1, trim=FALSE)(LikelihoodHousingHazard), " hazardous (estd.)", "<br/>",
...
map <- leaflet() %>%
  ...
  addLayersControl(
    baseGroups = c('Landlord types (Census 2011)', 'Hazardous housing (estd. 2019)'),
